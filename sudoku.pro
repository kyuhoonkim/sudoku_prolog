...
sudoku(Board) :-
	...
	%Columns
	columns(Row1,Row2,Row3,Row4,Row5,Row6,Row7,Row8,Row9), %Maps the all_distinct function to every col in the board which returns true if all integer values are unique
...
columns([],[],[],[],[],[],[],[],[]).
columns([Row1|T1],[Row2|T2],[Row3|T3],[Row4|T4],[Row5|T5],[Row6|T6],[Row7|T7],[Row8|T8],[Row9|T9]) :-
	all_distinct([Row1,Row2,Row3,Row4,Row5,Row6,Row7,Row8,Row9]),
	columns(T1,T2,T3,T4,T5,T6,T7,T8,T9).


% Checks the first value in all 9 lists for a completely unique column
% and continues check every column recursively
